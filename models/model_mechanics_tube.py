...
class Simulation:
    ...
    def egg_bound(self, pos):
        """
        Soft cylindrical wall in the xy-plane. Cells outside the ellipse
        (x / egg_shape[0])^2 + (y / egg_shape[1])^2 = 1 are penalized.
        z is unconstrained by this term.

        Parameters:
            pos (torch.Tensor): Input tensor of shape (N, 3) where N is the number of points.

        Returns:
            V_add_sum (float): Addition to the potential energy due to the tube wall.
        """
        axes = torch.tensor(self.egg_shape, device=pos.device, dtype=pos.dtype)
        pos_xy = pos[:, :2] / axes
        mag = torch.sum(pos_xy ** 2, dim=1)  # (x/ax)^2 + (y/ay)^2
        v_add = torch.where(mag > 1.0, (torch.exp(mag - 1.0) - 1) * 10., 0.)
        ...
        while True:
            ...
            if tstep % self.yield_every == 0 or len(x) > self.max_cells:    #Yield data if we are at a 'yield step' or if we have too many cells and the simulation is aborted
                ...
                if isinstance(p_mask, torch.Tensor):
                    ...
                    pp_mask = None
                yield xx, pp, qq, pp_mask                                  # Yielding the data 
...
def run_simulation(sim_dict):
    ...
    save((p_mask_lst, x_lst, p_lst,  q_lst), name='data'+str(sim_dict['init_number']), output_folder=output_folder)
    ...
    save((p_mask_lst, x_lst, p_lst,  q_lst), name='data'+str(sim_dict['init_number']), output_folder=output_folder)  # Last iteration is saved
