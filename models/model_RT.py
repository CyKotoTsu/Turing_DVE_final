...
class Simulation:
    ...
    def __init__(self, sim_dict):
        ...
        self.ceiling_threshold_bmp = sim_dict['ceiling_threshold_bmp']  # Distance from ceiling where Wnt sources are located
        self.offsets        = torch.tensor(sim_dict['offsets'], device=self.device, dtype=self.dtype)           # Offsets for periodic boundary conditions
        self.new_link = sim_dict['new_link']                      # Whether to use new link mechanism (Wnt --> DKK)
    ...
    def diffuse_morphogens(self, x, p_mask, c_dkk, c_wnt, 
        ...
        alpha_bmp (float): The source rate for BMP/Wnt.
        ...
        Theta = torch.zeros((len(x), 2), device=self.device, dtype=self.dtype) #Empty matrix for DKK and Wnt

        #In Theta matrix, select top cells near ceiling to be Wnt sources
        ceiling_cells = torch.where((x[:,1] > (self.ceiling_z - self.ceiling_threshold_bmp)))[0]

        Theta[ceiling_cells, 1] += alpha_bmp  # BMP source

        #All morphogens decay
        Theta[:, 0] += -U[:, 0]  # DKK decay
        Theta[:, 1] += -U[:, 1]  # Wnt decay
        ...
        diffusion = torch.zeros((len(x), 2), device=self.device, dtype=self.dtype)
        diffusion[:, 0] = -c_dkk * LU[:, 0]
        diffusion[:, 1] = -c_wnt * LU[:, 1]
    ...
    def egg_bound(self, pos):
        ...
        v_add = torch.where(mag > (1.0)**2, (torch.exp(mag - (1.0)**2) - 1)*10., 0.) # Calculate additional potential energy
...
def run_simulation(sim_dict):
    ...
    assert len(data_tuple) == 5 or len(data_tuple) == 2, 'data must be tuple of either len 2 (for data generation) or 5 (for data input)'
