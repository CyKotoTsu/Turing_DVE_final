...
class Simulation:
    ...
    def potential(self, x, p, q, p_mask, idx, tstep):
        ...
        if not torch.any(self.offsets):
            ...
            Vi = torch.sum(Vij, dim=1)  # Potential energy for each particle
    ...
    def time_step(self, x, p, q, p_mask, tstep, U):
        ...
        # Calculate potential
        V = self.potential(x, p, q, p_mask, idx, tstep)[0]
        v_plot = self.potential(x, p, q, p_mask, idx, tstep)[1]
        ...
        return x, p, q, p_mask, U, v_plot  #Returning the goods.
    ...
    def simulation(self, x, p, q, p_mask, U):
        ...
        while True:
            tstep += 1
            x, p, q, p_mask, U, v_plot = self.time_step(x, p, q, p_mask, tstep, U)        #Advancing the simulation one timestep
            ...
            if tstep % self.yield_every == 0 or len(x) > self.max_cells:    #Yield data if we are at a 'yield step' or if we have too many cells and the simulation is aborted
                ...
                VV = v_plot.detach().to("cpu").numpy().copy()
                if isinstance(p_mask, torch.Tensor):
                    pp_mask = p_mask.detach().to("cpu").numpy().copy()
                else:
                    pp_mask = None
                yield xx, pp, qq, pp_mask, UU, VV                                  # Yielding the data 
...
def run_simulation(sim_dict):
    ...
    save((p_mask_lst, x_lst, p_lst,  q_lst, U_lst, V_lst), name='data'+str(sim_dict['init_number']), output_folder=output_folder)
    ...
    for xx, pp, qq, pp_mask, UU, VV in itertools.islice(runner, yield_steps):
        i += 1
        if verbose:
    ...
    save((p_mask_lst, x_lst, p_lst,  q_lst, U_lst, V_lst), name='data'+str(sim_dict['init_number']), output_folder=output_folder)  # Last iteration is saved
